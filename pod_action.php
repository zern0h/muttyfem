    ...
    if($count > 0)
    {
      ...
      foreach($result as $row => $product){
        $id++;
          ...
          $output .= '<div class="col-md-3">';
            $output .= '<input type="date" placeholder="yyyy-mm-dd"  name="exp_date[]" id="exp_date'.$id.'" class="form-control" required value="" > ';
          $output .= '</div>';
  ...
  if($_POST['btn_action'] == 'Add')
  {
    ...
    if($result)
    {
      ...
      for($count = 0; $count<count($_POST["product_id"]); $count++)
        ...
        $pod_total_cost =   $_POST['quantity'][$count] * $_POST['pricePerUnit'][$count];

        $exp_date = $_POST['exp_date'][$count];
          ...
          if($inner_sub_count > 0){
            ...
            foreach ($inner_sub_result as $row => $product ){
              ...
              if($result)
              {
                ...
                $hist_action = $pod_product_quantity .' Units Received By';
                $former_level = $product['recorded_level'];
                $current_level = $newQuantity;
                $action_by_id = $receiving_staff_id;
